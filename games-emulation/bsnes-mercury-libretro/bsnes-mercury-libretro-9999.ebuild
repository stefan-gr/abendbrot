...
src_unpack() {
	...
	use profile_accuracy && LIBRETRO_CORE_NAME+=( bsnes_mercury_accuracy )
	use profile_balanced && LIBRETRO_CORE_NAME+=( bsnes_mercury_balanced )
	use profile_performance && LIBRETRO_CORE_NAME+=( bsnes_mercury_performance )
	libretro-core_src_unpack
}

src_compile() {
	append-cflags $(test-flags-CC -ffast-math)
	append-cxxflags $(test-flags-CXX -ffast-math)
	...
	mv out/bsnes_mercury_balanced_libretro.so .
	fi
	if use profile_performance; then
		...
		myemakeargs=( ${myemakeargs[@]/profile=*/} )
	# Never forget to move the file where the eclass expect it
	mv out/bsnes_mercury_performance_libretro.so .
	fi
	if use profile_accuracy; then
		...
		myemakeargs=( ${myemakeargs[@]/profile=*/} )
	# Never forget to move the file where the eclass expect it
	mv out/bsnes_mercury_accuracy_libretro.so .
	fi
}
